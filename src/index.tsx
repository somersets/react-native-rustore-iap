...
const RustoreIap = NativeModules.RustoreIap  ? NativeModules.RustoreIap  : new Proxy(
      {},
      {
        get() {
...
export function checkRuStoreAvailable(cb: (error: any, isAvailable: boolean) => void) {
  RustoreIap.checkRuStorePurchasesAvailability(cb);
}

export async function getRuStoreProducts(productsIds: String[]): Promise<RuStoreProduct[]> {
